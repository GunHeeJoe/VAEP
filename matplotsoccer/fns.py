...
def actions(
    ...
    for ty, r, loc, color, line in zip(action_type, result, location, colors, lines):
        ...
        a = f"({cnt}){ty}"
        if cnt == 1:
            plt.text(sx + text_offset, sy, a)
        elif cnt == 2:
            plt.text(sx-7 + text_offset, sy-6, a,color=color,alpha=0.5)
        elif cnt == 3:
            plt.text(sx-7 + text_offset, sy-5, a)
        elif cnt == 4:
            plt.text(sx-7.5 + text_offset, sy+6, a,color=color,alpha=0.75)
        elif cnt == 5:
            plt.text(sx+2 + text_offset, sy-4, a)
        elif cnt == 6:
            plt.text(sx-11 + text_offset, sy-4, a,color=color)
        elif cnt == 7:
            plt.text(sx-6 + text_offset, sy-6, a)
        elif cnt == 8:
            plt.text(sx-17 + text_offset, sy, a)
        elif cnt == 8:
            plt.text(sx + text_offset, sy+3, a)
        else:
            plt.text(sx + text_offset, sy, a)
